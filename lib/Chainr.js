...
Chainr.prototype.seq = function (name, fn) {
    ...
    this._order.push(new ExecutionContext.SequentialExecutionContext(this).push(new Call(name, fn)));
...
Chainr.prototype.par = function (name, fn) {
    ...
    if (last instanceof ExecutionContext.ParallelExecutionContext) {
        last.push(new Call(name, fn));
    } else {
        this._order.push(new ExecutionContext.ParallelExecutionContext(this).push(new Call(name, fn)));
    }

    return this;
};

Chainr.prototype.one = function (name, fn) {
    if (typeof name === 'function') {
        fn = name;
        name = null;
    }
    ...
    var last = this._peek();
    if (last instanceof ExecutionContext.OneExecutionContext) {
        last.push(new Call(name, fn));
    } else {
        this._order.push(new ExecutionContext.OneExecutionContext(this).push(new Call(name, fn)));
    }
...
Chainr.prototype.catch = function (fn) {
    this._order.push(new ExecutionContext.CatchExecutionContext(this).push(new Call(null, fn)));

    return this;
};

Chainr.prototype.addToContext = function (name, values) {
    if (name) {
        this.vars[name] = values[0];
    } else {
        this._unnamedVars = this._unnamedVars.concat(values);
    }
};

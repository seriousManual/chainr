...
Chainr.prototype.catch = function (fn) {
    var tmp = new ExecutionContext.CatchExecutinoContext(this);
    tmp.push(null, fn);
    this._order.push(tmp);
...
Chainr.prototype._getNext = function () {
    ...
    if (!next) return null;
    if (next instanceof ExecutionContext.CatchExecutinoContext) return this._getNext();
...
Chainr.prototype._getNextCatch = function () {
    ...
    if (!next) return null;
    if (!(next instanceof ExecutionContext.CatchExecutinoContext)) return this._getNextCatch();
